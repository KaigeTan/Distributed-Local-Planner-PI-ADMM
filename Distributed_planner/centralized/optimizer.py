...
class OBCAOptimizer:
    ...
    def initialize(self, t_step, init_state, max_x, max_y):
        ...
        self.x0 += [[0.1]*(self.num_veh*self.n_dual_variable*self.N_horz)] # TODO: check dual variable number, dual variable initialization
        self.max_x = max_x
        self.max_y = max_y
    ...
    def build_model(self) -> bool:
        ...
        self.rhs = ca.vertcat(ca.vertcat(v*ca.cos(theta+beta), v*ca.sin(theta+beta),
                                   a, v/self.lr*ca.sin(beta)), steering_rate) # 5*num_veh X 1, TODO: doublecheck the last-but-two term
    ...
    def generate_constrain(self):
        ...
        for i_tstep in range(1, self.N_horz): # iterate each predicted time horizon
            ...
            for i_veh in range(self.num_veh):
                veh_vertices = generate_vehicle_vertices(self.X[i_tstep][i_veh, [0, 1, 3]]) # 4 x 2
                A_temp, b_temp = compute_square_halfspaces_ca(veh_vertices)
                A += [A_temp] # 4 X 2
                b += [b_temp] # 4 X 1
                lamb += [self.LAMBDA[i_tstep][i_veh, :].T] # 4 X 1
