...
class OBCAOptimizer:
    ...
    def local_generate_constrain(self):
        ...
        for i_tstep in range(1, self.N_horz): # iterate each predicted time horizon
            ...
            self.constrains += [-b.T@self.bar_state.lamb[self.veh_idx, i_tstep-1, :]
                                -np.dot(self.bar_state.b[1-self.veh_idx, i_tstep, :], 
                                        self.bar_state.lamb[1-self.veh_idx, i_tstep-1, :])] # (5a)
            self.lbg += [self.min_dis] # minimal distance req.
            self.ubg += [1000]
            self.constrains += [A.T@self.bar_state.lamb[self.veh_idx, i_tstep-1, :] +\
                                self.bar_state.A[1-self.veh_idx, i_tstep].T@self.bar_state.lamb[1-self.veh_idx, i_tstep-1, :]] # (5b)
            self.lbg += [0, 0]
            self.ubg += [0, 0]
        
        for i_tstep in range(0, self.N_horz-1):
            self.constrains += [(A.T@self.bar_state.lamb[self.veh_idx, i_tstep, :]).T@
                                (A.T@self.bar_state.lamb[self.veh_idx, i_tstep, :])]
            self.lbg += [0]
            self.ubg += [1]

    def bar_state_update(self, bar_x):
        ...
        for i_tstep in range(self.N_horz): # iterate each predicted time horizon
            for i_veh in range(self.num_veh):
                # generate polytopic set if vehicles at i_tstep
                if self.prob:
                    A_temp, b_temp = compute_square_halfspaces_ca_prob(np.array(bar_x[i_veh][i_tstep, :]).ravel())
                else:
                    veh_vertices_bar = generate_vehicle_vertices(np.array(bar_x[i_veh][i_tstep, :]).ravel()) # 4 x 2
    ...
    def edge_initialize(self, max_x, max_y):
        ...
        for i_t in range(1, self.N_horz):
            self.x0 += [[0]*2*self.n_dual_variable] # LAMBDA at a single time step, 1 X 2*self.n_dual_variable
        self.max_x = max_x
        self.max_y = max_y
    ...
    def edge_solve(self):
        ...
        u_opt = sol['x']
        # get optimized result
        lambda_bar = np.array(np.round(u_opt, decimals=4))
        # TODO: double check the result
        # annoying, have to use two steps to split into 3d array
        lambda_bar = np.reshape(lambda_bar, (self.N_horz-1, self.num_veh*self.n_dual_variable)) # time-step wise
        lambda_bar = np.reshape(lambda_bar, (self.N_horz-1, self.num_veh, self.n_dual_variable), 'F') # N_horz X num_veh X n_dual_variable
    ...
    def edge_generate_constrain(self):
        ...
        for i_tstep in range(self.N_horz-1):
            self.constrains += [self.bar_state.A[0, i_tstep+1].T@self.LAMBDA[i_tstep][0, :].T +
                                self.bar_state.A[1, i_tstep+1].T@self.LAMBDA[i_tstep][1, :].T] # 2 x 1
            self.lbg += [0, 0]
            self.ubg += [0, 0]
            self.constrains += [-(self.bar_state.b[0, i_tstep+1][np.newaxis, :])@self.LAMBDA[i_tstep][0, :].T
                                -(self.bar_state.b[1, i_tstep+1][np.newaxis, :])@self.LAMBDA[i_tstep][1, :].T] # (6c-1)
            self.lbg += [self.min_dis] # minimal distance req.
            self.ubg += [1000]
            self.constrains += [(self.bar_state.A[0, i_tstep+1].T@self.LAMBDA[i_tstep][0, :].T).T@
                                (self.bar_state.A[0, i_tstep+1].T@self.LAMBDA[i_tstep][0, :].T)]
            self.lbg += [0]
            self.ubg += [1]
            self.constrains += [(self.bar_state.A[1, i_tstep+1].T@self.LAMBDA[i_tstep][1, :].T).T@
                                (self.bar_state.A[1, i_tstep+1].T@self.LAMBDA[i_tstep][1, :].T)]
            self.lbg += [0]
            self.ubg += [1]
    ...
    def edge_generate_object(self):
        ...
        for i in range(self.N_horz-1):
            ...
            self.obj += (self.bar_state.b[0, i+1][np.newaxis, :])@(self.LAMBDA[i][0, :].T) + \
                        (self.bar_state.b[1, i+1][np.newaxis, :])@(self.LAMBDA[i][1, :].T)
    ...
    class mid_state:
        def __init__(self, outer):
            self.lamb = np.zeros((outer.num_veh, outer.N_horz-1, outer.n_dual_variable)) # 2x10x4
