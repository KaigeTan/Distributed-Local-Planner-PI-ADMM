...
class OBCAOptimizer:
    ...
    def local_generate_constrain(self, t_step):
        ...
        for i_tstep in range(1, self.N_horz): # iterate each predicted time horizon
            ...
            if self.veh_idx == 0:
                ...
                temp_sgn = -1    
            self.constrains += [A.T@self.bar_state.lamb_ij[self.veh_idx, i_tstep-1, :] + temp_sgn*self.bar_state.s[i_tstep-1]] # (5b)
            self.lbg += [0, 0]
            self.ubg += [0, 0]
    ...
    def edge_initialize(self, max_x, max_y):
        ...
        for i_t in range(1, self.N_horz):
            ...
            self.z0 += [[0]*self.n_s] # S at a single time step, 1 X n_s            
        self.max_x = max_x
        self.max_y = max_y
    ...
    def edge_build_model(self) -> bool:
        ...
        self.S = ca.SX.sym('S', self.n_s, self.N_horz-1)
        self.obj = 0
        return True
    ...
    def edge_generate_constrain(self):
        ...
        for i_tstep in range(self.N_horz-1):
            ...
            self.constrains += [self.bar_state.A[0, i_tstep, :, :].T@self.Z[i_tstep][0, :].T +
                                self.S[:, i_tstep]] # 2 X 1
            self.lbg += [0, 0]
            self.ubg += [0, 0]
            # (6b)
            self.constrains += [self.bar_state.A[1, i_tstep, :, :].T@self.Z[i_tstep][1, :].T -
                                self.S[:, i_tstep]] # 2 X 1
            self.lbg += [0, 0]
            self.ubg += [0, 0]
            # (6c-1)
    ...
    def edge_generate_variable(self):
        ...
        for i in range(self.N_horz-1):
            ...
            self.lbx += [1e-3*0]*self.num_veh*self.n_loc_lambda
            self.ubx += [1e4]*self.num_veh*self.n_loc_lambda
        
        # variables are slack variables, N_horz-1 X 2
        for i in range(self.N_horz-1):
    ...
    def edge_solve(self):
        ...
        opts = {'ipopt.max_iter':10000, 
                ...
                'ipopt.acceptable_obj_change_tol':1e-5}
        solver = ca.nlpsol('solver', 'ipopt', nlp_prob) # , opts
        sol = solver(lbx=self.lbx, ubx=self.ubx,
                     ubg=self.ubg, lbg=self.lbg) # x0=ca.vertcat(*self.z0), 
        u_opt = sol['x']
        # get optimized result
        u_opt = np.array(np.round(u_opt, decimals=4))
        # TODO: double check the result
        lamb_num = self.num_veh*self.n_loc_lambda*(self.N_horz-1)
        # annoying, have to use two steps to split into 3d array
        Z_bar = np.reshape(u_opt[:lamb_num], (self.N_horz-1, self.num_veh*self.n_loc_lambda)) # time-step wise
        ...
        self.bar_state.s = np.reshape(u_opt[lamb_num:], (self.N_horz-1, self.n_s))
    ...
    class mid_state:
        def __init__(self, outer):
            ...
            self.s = np.zeros((outer.N_horz-1, outer.n_s))
