...
class VehicleConfig:   
    ...
    def ref_traj_gen_intersection(self, num_veh = 2):
        v1 = 10 # m/s
        x1 = np.linspace(0, 0+v1*self.T, int(self.T/self.dt)+1) 
        v2 = 10 # m/s
        y2 = np.linspace(v2*self.T/2, -v2*self.T/2, int(self.T/self.dt)+1) 
        # ref trajectory, T/dt X 5: x, y, v, head, steer
        ref_traj1 = np.vstack((x1,
                               np.zeros_like(x1), 
                               v1*np.ones_like(x1),
                               np.zeros_like(x1), 
                               np.zeros_like(x1))).T
        ref_traj2 = np.vstack((v1*self.T/2*np.ones_like(y2),
                               y2,
                               v2*np.ones_like(y2),
